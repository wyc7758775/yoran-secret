  ...
  try {
    if (
      ...
      tocItems.value = Array.from(headers).map((header, index) => {
        const level = Number.parseInt(header.tagName.replace('H', ''), 10)
...
function scrollToSection(item: TocItem, event: MouseEvent) {
  if (typeof window !== 'undefined' && typeof document !== 'undefined') {
    event.preventDefault()

    const headers = document.querySelectorAll(querySelectors())
    const actualElement = Array.from(headers).find(

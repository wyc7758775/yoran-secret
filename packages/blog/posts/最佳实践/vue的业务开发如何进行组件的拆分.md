...
依旧是A B C 三个组件.但是A B C三个组件外边飘的那些箭头不存在了. 所有能够控制它们的内部状态的方式都集中在了`controllers`上面. 

其中`controllers`部分的组织形式和vue的`composition api`宣传图表现一致。
...
`cController.js`就是`controllers`中的一个`void`. 引入到A组件当中,然后将里面的方法通过props传给B组件.
...
也就是说,控制C组件内部状态的是通过引入到A组件中的`controller`来进行通过,中间的B组件不做任何的处理,仅仅作为一个中转站. 操作起来和理论都很简单。但是想要更好的拆分的话，还需要了解三个概念：
...
控制目录树的相关逻辑都放在`listTreeController`控制器里边, 和右边内容content相关逻辑都放入到`renderContentController`的方法当中.

![](https://cdn.nlark.com/yuque/0/2022/png/654315/1670061591951-06c51152-80b2-4355-bc3a-bfdbaf15b6d2.png)

随后将controller中公共方法都传进到组件当中. `doc-aside`是包括`search`和`tree`已经`other`三个模块的中转组件. 不在这个组件中做任何的逻辑处理. 如下图:
...
+ 所有功能点都受控挂载在vuex的store上面的一个变量, 没有权限的话,就直接通过`v-if`来隐藏对应的入口
...
> <font style="color:rgb(48, 49, 51);">隐藏和显示的渐入渐出效果是elementUI框架内置的.</font>
>
...
通过通过接口拿到的,或者自己组件的数据传进来之后,再进行对`v-model`的控制. `data.id`这样的变量就是业务变量, 通过业务变量来直接控制UI的组件的显示和隐藏,就是业务变量和UI变量的混用. 或者说**业务逻辑和交互逻辑的混用. **
...
其中`ishow`和`type` 就可以视为UI变量, 它们不关心外界是通过了什么判断, 只关系传进来的是true还是false.
...
1. 实在是太多可以使用vue的`$attr`和 `$listeners`
2. 为了维护对于数据的溯源
